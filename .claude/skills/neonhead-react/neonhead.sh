...
cue() {
    ...
    case "$kind" in
        ...
        title)
            ...
            ;;
        say)
            # text may contain spaces - take everything from $2 on. Mouth-
            # only viseme animation (text_to_timeline in app.py), no audio.
            send "{\"type\":\"say\",\"text\":\"$(json_escape "${*:2}")\"}"
            ;;
    esac
}
...
case "$cmd" in
    ...
    *) echo "usage: $0 start|stop|status|emotion NAME [WEIGHT]|gesture NAME|title TEXT|say TEXT" >&2; exit 1 ;;
esac