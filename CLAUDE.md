...
a TTS pipeline) or the bundled CLI client.
...
from a wav + known text (letter-to-viseme), for use with TTS pipelines
that give no phoneme timings of their own.
...
floats in `Config.rgb`. See README.md's "Config" section for what each
